...
All visualizers MUST implement comprehensive event tracking using the standardized utility hooks in `hooks/useEventTracking.ts`. This modern approach eliminates boilerplate code, provides type safety, and ensures consistent event structure across all visualizers.

## Quick Start Guide
The new event tracking system reduces implementation from 50+ lines to just 3-5 lines per visualizer:

```typescript
import { useLifecycleEvents, useCommonEventHandlers } from '../../../hooks/useEventTracking';

export default function YourVisualizer() {
  // Automatic lifecycle tracking (component load/unload)
  useLifecycleEvents('your-route-name');

  // Pre-built common event handlers
  const { trackClick, trackFormInput, trackAlgorithmStart, trackAlgorithmStep, trackAlgorithmComplete } = useCommonEventHandlers('your-route-name');

  // Use in your handlers
  const handleStart = () => {
    trackClick('start-clicked');
    trackAlgorithmStart({ arrayLength: array.length });
...
### `useLifecycleEvents(visualizerName: string)`
Automatically tracks component lifecycle events with zero configuration:

```typescript
import { useLifecycleEvents } from '../../../hooks/useEventTracking';

export default function YourVisualizer() {
  // Single line setup - tracks component load/unload and time spent
  useLifecycleEvents('binary-search');
...
- `{visualizer-name}-loaded` - Component mount
- `{visualizer-name}-unloaded` - Component unmount
- `{visualizer-name}-time-spent` - Time spent on component

### `useCommonEventHandlers(visualizerName: string)`
Pre-built handlers for the most common visualizer interactions:

```typescript
import { useCommonEventHandlers } from '../../../hooks/useEventTracking';

export default function YourVisualizer() {
  const {
    trackClick,              // Button clicks and interactions
    trackFormInput,         // Input field changes
    trackAlgorithmStart,    // Algorithm execution start
    trackAlgorithmStep,     // Step-by-step progression
    trackAlgorithmComplete, // Algorithm completion/failure
  } = useCommonEventHandlers('binary-search');
}
```

### `useEventTracker(visualizerName: string)`
Low-level hook for custom event tracking:

```typescript
import { useEventTracker } from '../../../hooks/useEventTracking';

export default function YourVisualizer() {
  const { trackEvent } = useEventTracker('binary-search');

  // Custom event tracking
  const handleCustomAction = () => {
    trackEvent('CLICK', 'custom-action', { customData: 'value' });
  };
}
```

### `createEventHandler()`
HOC for wrapping existing functions with event tracking:

```typescript
import { createEventHandler, useEventTracker } from '../../../hooks/useEventTracking';

export default function YourVisualizer() {
  const { trackEvent } = useEventTracker('binary-search');

  const originalHandler = () => { /* existing logic */ };
  const wrappedHandler = createEventHandler(trackEvent, 'CLICK', 'wrapped-action', originalHandler);
}

## Event Naming Convention
Event names are automatically generated using the pattern: `{visualizer-name}-{action}`

**Visualizer Name Rules:**
- Use the exact kebab-case route name from your URL path
- Examples: `binary-search`, `two-pointer`, `heap-sort`, `topological-sort`

**Action Names:**
- Automatically generated by hooks for common actions
- Custom actions should be descriptive and kebab-case

**Examples:**
```typescript
// Automatic lifecycle events (via useLifecycleEvents)
"binary-search-loaded"
"two-pointer-unloaded"
"heap-sort-time-spent"

// Common handler events (via useCommonEventHandlers)
trackClick('start-clicked')        → "binary-search-start-clicked"
trackFormInput('array-input')      → "binary-search-array-input"
trackAlgorithmStart()              → "binary-search-algorithm-started"
trackAlgorithmStep()               → "binary-search-step-executed"
trackAlgorithmComplete(true)       → "binary-search-algorithm-completed"

// Custom events (via useEventTracker)
trackEvent('CLICK', 'node-selected', data) → "binary-search-node-selected"
```

## Implementation Patterns

### 1. Basic Setup (Required)
Every visualizer MUST include lifecycle tracking:

```typescript
import { useLifecycleEvents } from '../../../hooks/useEventTracking';

export default function YourVisualizer() {
  // Automatic lifecycle tracking - REQUIRED
  useLifecycleEvents('your-route-name');
...
Track user interactions with pre-built handlers:
...
export default function YourVisualizer() {
  useLifecycleEvents('binary-search');
  const { trackClick } = useCommonEventHandlers('binary-search');
  ...
  return (
    <NSButton onClick={handleStart}>Start</NSButton>
    <NSButton onClick={handleReset}>Reset</NSButton>
  );
}
```
...
export default function YourVisualizer() {
  const { trackFormInput } = useCommonEventHandlers('binary-search');

  const handleArrayInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;

    trackFormInput('array-input', {
...
### 4. Algorithm Execution Tracking
Track algorithm progress with specialized handlers:
...
export default function YourVisualizer() {
  ...
  const executeStep = () => {
    trackAlgorithmStep({
      ...
      rightPointer: right,
      currentSum: sum,
      action: 'move-left' // or 'move-right', 'found', etc.
    });
  ...
  const onAlgorithmComplete = (success: boolean) => {
    trackAlgorithmComplete(success, {
...
For specialized interactions not covered by common handlers:
...
export default function YourVisualizer() {
  ...
  const handleVisualizationModeChange = (mode: string) => {
    trackEvent('VIEW', 'mode-changed', { mode });
...
The event tracking hooks provide complete type safety:

```typescript
import type { EventType } from '../../../hooks/useEventTracking';

// All event types are strictly typed
const eventType: EventType = 'CLICK'; // ✅ Valid
const invalidType: EventType = 'INVALID'; // ❌ TypeScript error

// Event data uses Record<string, unknown> for type safety
...
// Avoid any types
const badData: any = { /* ... */ }; // ❌ Avoid this
```

### Performance Considerations
The utility hooks are optimized for performance:

```typescript
// ✅ Hooks use useCallback internally
const { trackClick } = useCommonEventHandlers('binary-search');

// ✅ Event tracking doesn't cause re-renders
const handleClick = () => {
  trackClick('button-clicked'); // No re-render triggered
  // Your state updates here
};

// ✅ Lifecycle events are automatically cleaned up
useLifecycleEvents('binary-search'); // Handles cleanup automatically
```

### Error Handling
Event tracking is designed to be non-blocking:

```typescript
// Events never throw errors or block your application
trackClick('action'); // Always safe to call
trackAlgorithmStart(data); // Handles malformed data gracefully
```

## Mandatory Implementation Checklist

For every visualizer, ensure these steps are completed:

- [ ] **Import event tracking hooks** - `useLifecycleEvents` and/or `useCommonEventHandlers`
- [ ] **Add lifecycle tracking** - Single line: `useLifecycleEvents('route-name')`
- [ ] **Track button interactions** - Use `trackClick()` for Start, Reset, Next, Previous buttons
- [ ] **Track input changes** - Use `trackFormInput()` for array input, parameters, dropdowns
- [ ] **Track algorithm events** - Use `trackAlgorithmStart()`, `trackAlgorithmStep()`, `trackAlgorithmComplete()`
- [ ] **Test in browser console** - Verify events are fired correctly during development

### Route Name Validation
Ensure you use the correct kebab-case route name:

```typescript
// ✅ Correct - matches /binary-search route
useLifecycleEvents('binary-search');

// ✅ Correct - matches /topological-sort route
useLifecycleEvents('topological-sort');

// ❌ Incorrect - doesn't match route
useLifecycleEvents('binarySearch');
useLifecycleEvents('binary_search');
```
...
const TopicNameVisualizer: React.FC = () => {
  ...
  // Event tracking - Modern approach (replaces 20+ lines of manual postMessage calls)
  useLifecycleEvents('topic-name');
  const { trackClick, trackFormInput, trackAlgorithmStart } = useCommonEventHandlers('topic-name');

  const handleStart = () => {
    trackClick('start-clicked');
    trackAlgorithmStart({ inputLength: inputValue.length });
    // Your algorithm logic...
  };
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
    setInputValue(value);
    trackFormInput('input-changed', { length: value.length });
  };
...
- [ ] **Event tracking hooks imported** - `useLifecycleEvents` and `useCommonEventHandlers`
- [ ] **Lifecycle tracking added** - Single line: `useLifecycleEvents('route-name')`
- [ ] **Button interactions tracked** - `trackClick()` for all major buttons
- [ ] **Form inputs tracked** - `trackFormInput()` for input fields and dropdowns
- [ ] **Algorithm events tracked** - `trackAlgorithmStart()`, `trackAlgorithmStep()`, `trackAlgorithmComplete()`
- [ ] **Route name validation** - Uses exact kebab-case route name
- [ ] **Events tested in browser console** - Verify events fire during development

### Configuration & Deployment
- [ ] **Configuration updated in `data/visualizers.json`**
...
### Quality Assurance
- [ ] TypeScript compilation passes without `any` types
- [ ] All user interactions properly tracked
- [ ] Algorithm progression events captured
- [ ] No performance impact on visualization
- [ ] Error handling tested (malformed inputs)
